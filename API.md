...
Base path: `/api/ai/sessions`

Fitur chatbot AI berbasis natural language. User mengirim pertanyaan dalam bahasa natural, system mengkonversinya ke SQL, mengeksekusi ke database POS, dan mengembalikan insight dalam bahasa natural.

**Arsitektur:** Express bertindak sebagai gateway — mengelola sesi dan autentikasi, lalu meneruskan pertanyaan ke FastAPI AI service (internal).

**Scope percakapan:**

| Scope | Keterangan |
|-------|-----------|
| `global` | Konteks semua toko (untuk superAdmin) |
| `shop` | Konteks toko tertentu, wajib sertakan `shop_id` |

---

### POST /api/ai/sessions

**Role:** semua · Buat sesi percakapan baru.

**Body (JSON):**

| Field | Tipe | Wajib | Keterangan |
|-------|------|-------|-----------|
| `scope` | `global` \| `shop` | ✓ | Konteks percakapan |
| `shop_id` | UUID | Jika `scope = shop` | ID toko yang menjadi konteks |
...
```json
{
  ...
  "data": {
    ...
    "scope": "shop",
    "shop_id": "shop-uuid-1111",
    "last_active_at": "2026-05-10T08:00:00.000Z",
...
**Role:** semua · Daftar sesi percakapan milik user yang sedang login.

**Query params:**

| Parameter | Tipe | Keterangan |
|-----------|------|-----------|
| `scope` | `global` \| `shop` | Filter berdasarkan scope |
| `page` | number | |
| `limit` | number | |
  ...
  "data": [
    {
      ...
      "scope": "shop",
      "shop_id": "shop-uuid-1111",
      "last_active_at": "2026-05-10T09:30:00.000Z",
      "created_at": "2026-05-10T08:00:00.000Z",
      "Shop": { "id": "shop-uuid-1111", "name": "Segar Jaya Pusat" }
    }
  ],
...
> Diurutkan berdasarkan `last_active_at` terbaru. Sesi yang sudah dihapus tidak ditampilkan.

---

### GET /api/ai/sessions/:sessionId

**Role:** semua · Detail satu sesi beserta seluruh riwayat pesan.

> User hanya bisa mengakses sesi miliknya sendiri.
...
```json
{
  ...
  "data": {
    ...
    "messages": [
      {
        "id": "msg-uuid-aaaa",
        "session_id": "sess-uuid-1111",
        "role": "user",
        "content": "Produk apa yang paling laris bulan ini?",
        "created_at": "2026-05-10T08:05:00.000Z"
      },
      {
        "id": "msg-uuid-bbbb",
        "session_id": "sess-uuid-1111",
        "role": "assistant",
        "content": "SELECT pv.name AS produk, SUM(ti.qty) ...",
        "created_at": "2026-05-10T08:05:03.000Z"
      }
    ]
  }
}
...
> `messages` diurutkan dari pesan paling lama ke terbaru.  
> Pesan `assistant` berisi SQL (bukan insight), karena SQL digunakan sebagai konteks untuk pertanyaan lanjutan.

---

### DELETE /api/ai/sessions/:sessionId

**Role:** semua · Hapus sesi (soft delete).

> User hanya bisa menghapus sesi miliknya sendiri.
...
### POST /api/ai/sessions/:sessionId/chat

**Role:** semua · Kirim pertanyaan ke AI dan dapatkan jawaban.

Pipeline yang dijalankan: **NL → SQL → Eksekusi PostgreSQL → Insight**

> User hanya bisa mengirim pesan ke sesi miliknya sendiri.  
> History percakapan diambil dan disimpan otomatis oleh AI service.

**Body (JSON):**

| Field | Tipe | Wajib | Keterangan |
|-------|------|-------|-----------|
| `question` | string | ✓ | Pertanyaan dalam bahasa natural (3–500 karakter) |

**Contoh request:**
```json
{
  "question": "Produk apa yang paling laris bulan ini?"
}
```
...
{
  "success": true,
  "message": "Query berhasil diproses",
  "data": {
    "question": "Produk apa yang paling laris bulan ini?",
    "sql": "SELECT pv.name AS produk, SUM(ti.qty) AS total_terjual FROM transaction_items ti JOIN product_variants pv ON ti.product_variant_id = pv.id JOIN transactions t ON ti.transaction_id = t.id WHERE t.status = 'completed' AND DATE_TRUNC('month', t.paid_at) = DATE_TRUNC('month', NOW()) GROUP BY pv.id, pv.name ORDER BY total_terjual DESC LIMIT 10",
    "columns": ["produk", "total_terjual"],
    "rows": [["Kopi Susu 500ml", 342], ["Teh Tarik", 210]],
    "row_count": 2,
    "insight": "Produk terlaris bulan ini adalah Kopi Susu 500ml dengan 342 unit terjual.",
    "sql_latency_ms": 2340,
    "insight_latency_ms": 820,
    "total_latency_ms": 3160
  }
}
```
...
| Kode | Kondisi |
|------|---------|
| `200` | Pipeline berhasil |
| `400` | Question kosong |
| `401` | Token tidak valid |
| `404` | Session tidak ditemukan atau bukan milik user |
| `422` | SQL yang digenerate tidak valid atau error saat eksekusi |
| `502` | AI service tidak dapat dihubungi |
